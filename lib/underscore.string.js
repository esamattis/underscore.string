...
(function(root){
  ...
  var _s = {
     ...
     * keep original string if truncated + pruneStr goes over original length
    ...
    prune: sArgs(function(str, length, pruneStr){
      ...
      if (str.length <= length+pruneStr.length)
        return str;

      // Check if we're in the middle of a word
      if( str.substring(length-1, length+1).search(/^\w\w$/) === 0 )
        str = _s.rtrim(str.slice(0,length).replace(/([\W][\w]*)$/,''));
      else
        str = _s.rtrim(str.slice(0,length));
